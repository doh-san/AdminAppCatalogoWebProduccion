              ...
              <div class="card-body card-body-cascade text-center">
                ...
                <form class="text-center col-md-10 offset-md-1" method="POST" action="{{ route('productos.update', $prod->id_producto) }}" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6">
                      <div class="md-form mb-0">
                         <select class="mdb-select md-form colorful-select dropdown-ins" name="marca">
                           <option value="" disabled selected>Elige tu opción</option>
                           @foreach ($marcas as $marca)
                             @foreach ($marca->producto_marca as $pm)
                               <option value="{{ $marca->id_marca }}" {{ old('marca') == $marca->id_marca ? 'selected' : '' }} {{ $prod->id_marca == $marca->id_marca ? 'selected' : '' }}>{{ $marca->nombre . ' - ' . $pm->fecha }}</option>
                             @endforeach
                           @endforeach
                         </select>
